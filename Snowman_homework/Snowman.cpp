  ...
  mEyePosW(0.0f, 0.0f, 0.0f), mLandIndexCount(0), mTheta(1.3f*MathHelper::Pi), mPhi(0.4f*MathHelper::Pi), mRadius(80.0f)
{
	...
	XMMATRIX snowmanScale = XMMatrixScaling(1.0f, 1.0f, 1.0f);
	XMMATRIX snowmanOffset = XMMatrixTranslation(0.0f, 1.0f, 0.0f);
	XMStoreFloat4x4(&mSnowmanWorld, XMMatrixMultiply(snowmanScale, snowmanOffset));
...
void SnowmanApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p) {
		...
		XMMATRIX worldViewProj = world * view*proj;
		// Set per object constants.
		world = XMLoadFloat4x4(&mSnowmanWorld);
...
void SnowmanApp::BuildSnowmanGeometryBuffers()
{
	...
	geoGen.CreateSphere(5.0f,64,64,snowman);
